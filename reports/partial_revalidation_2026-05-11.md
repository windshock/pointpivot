...
- `121.170.203.142`는 최근 스팸 평판(CleanTalk `last activity=2026-05-08`)과 같은 `/24`의 `121.170.203.144` 직접 IOC 때문에 follow-up 가치가 높다.
- 다만 `121.170.203.142` 자체를 `@rnfma9`/USIM 캠페인과 직접 연결하는 작성자 IP 원문은 확보하지 못했다.
...
| 직접 확인 | `121.170.203.142` dry-run, CleanTalk 원문, `dongponews.net` 후보 원문, `121.170.203.144` 관련 XE 원문 grep |
...
| 합계 | 75 | 51 | 6 | 원문 직접 증거 신규 승격 0 |

## 핵심 IP별 결과

| IP | 결과 | 판정 |
|---|---|---|
| `121.170.203.142` | DDG exact 결과는 AbuseIPDB/CleanTalk/CriminalIP/IP 디렉터리 중심. CleanTalk 원문은 blacklisted 및 `last activity=2026-05-08`를 표시. `@rnfma9`/USIM 캠페인 직접 원문은 미확인 | `PARTIAL/LOW` 유지. 같은 `/24`의 `121.170.203.144`와 함께 follow-up |
...
`PARTIAL` 전수 재검증은 필요하다. 특히 Google/검색엔진 스니펫에는 삭제된 게시글의 핸들·작성자 IP가 남을 수 있어 `PARTIAL` 큐를 주기적으로 재확인해야 한다. 다만 이번 배치에서는 신규 DONE/HIGH 승격 가능한 원문 직접 증거는 없었다. `121.170.203.142`는 최근 평판과 같은 `/24` direct IOC 때문에 계속 감시하되, 현재는 `121.170.203.144`와 동일 행위자로 귀속하지 않는다.
