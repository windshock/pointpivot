# PointPivot 현황 요약 (Summary)

> 자동 생성: 2026-05-11

---

## 조사 커버리지 (seed IP 기준)

| 서비스 | 전체 | DONE | PARTIAL | UNVERIFIED | 1차 스캔 커버리지 |
|---|---|---|---|---|---|
| 서비스A | 8 | 2 | 6 | 0 | 100% |
| 기프티콘 | 25 | 1 | 24 | 0 | 100% |
| 서비스C | 32 | 0 | 32 | 0 | 100% |
| **합계** | **65** | **3** | **62** | **0** | **100%** |

> **1차 스캔 커버리지** = DONE + PARTIAL (DDG 검색·초안 보고서까지 완료된 비율).  
> **확정률 (DONE)** = 3/65 (5%) — 직접 게시 증거 또는 복수 출처로 확인 완료.
> 서비스별 합계는 중복 IP를 서비스마다 계산하며, 위협 seed unique 기준은 64개.
> **제휴사 POS 오탐 검증용 185개**는 위협 seed 진행률에서 제외.

---

## 추적 IP 및 클러스터 배분

| 클러스터 | 상태 | IP 수 | 핵심 IOC |
|---|---|---|---|
| Cluster#1 | 🔴 활성 | 9 | @brrsim_77, @abab1768, @the_usim (내구제/유심 스팸) |
| Cluster#2 | 🟡 부분 확인 | 30 | @YY77882 (불법 의약품 자동화, Vultr VPS) |
| Cluster#3 | 🟡 부분 확인 | 14 | @GO174 (먹튀/도박/통장협박 스팸, KT 118.235.x.x) |
| 미분류 | 추가 조사 필요 | 29 | 기프티콘 KR_RESIDENTIAL 등 — 클러스터 미배정 |
| **전체** | | **85** | 위협 seed 64개(unique) + 피벗 21개(unique) |

---

## 식별된 IOC

| 유형 | 개수 | 비고 |
|---|---|---|
| 텔레그램 핸들 | 23 | DONE/PARTIAL: 10 |
| 도메인/URL | 10 | isweb.co.kr 기반 홍보 사이트 |
| 피해 사이트 | 31 | 스팸 게시글 확인된 사이트 수 |

---

## IOC 원본 경로

- **IP 전체 목록·상태:** [`investigations/INDEX.md`](../investigations/INDEX.md)
- **텔레그램·도메인·닉네임:** [`data/ioc_registry.md`](../data/ioc_registry.md)
- **클러스터 상세:** [`data/campaigns.md`](../data/campaigns.md)
- **피해 사이트:** [`data/spammed_sites.md`](../data/spammed_sites.md)

> 표준 포맷 export(STIX/CSV)는 DONE IP가 충분히 확보된 뒤 계획.

---

## 다음 우선 작업

→ [`STATUS.md`](../STATUS.md) 참조
