...
## 5. 조사 시 주의사항 및 증거 검증 원칙

### 5.1 증거 검증의 엄격성 (Anti-Hallucination)
- **AI 요약 맹신 금지:** 검색 도구(예: Google Search)가 제공하는 AI 요약이나 단편적인 키워드 조합은 일반론적인 위험성을 설명하는 경우가 많다. 특정 IP가 악성이라고 단정하기 전, 반드시 해당 IP 주소가 악성 행위의 '주체'로 명시된 보안 리포트 원문이나 스팸 게시글 스니펫을 직접 확인해야 한다.
- **검색 window와 게시일 분리:** 최근 7일 검색 결과에 노출됐더라도 원문 게시일은 과거일 수 있다. 최근 활동으로 판정하려면 원문 `datePublished`/`작성일`/캡처 시점이 window 안에 있어야 한다. 반복 점검은 `scripts/recent_window_check.py`를 사용한다.
- **검색 provider 결과의 한계:** DDG/SearXNG/hybrid 검색 결과는 Tier-1 후보 수집 신호이며 기본 증거 등급은 `search_snippet_only`다. provider/engine이 달라도 원문 확인 전에는 직접 증거가 아니며, DONE/HIGH 판정이나 텔레그램 핸들 자동 등록 근거로 사용하지 않는다.
- **Public proxy 한계:** ProxyGather 기반 proxy pool은 SearXNG `outgoing.proxies` 후보 공급원일 뿐이며, 모든 proxy는 로컬 검증 전까지 신뢰하지 않는다. Proxy 경유 검색 결과도 Tier-1 `search_snippet_only` 후보로만 취급하고, CAPTCHA·Cloudflare·로그인·paywall·명시적 차단 우회나 query rate 증폭에 사용하지 않는다. Public proxy에는 credential, cookie, token, 비공개 조사 메모 등 민감 정보를 보내지 않는다.
- **인프라와 행위자의 분리:** 네이버 클라우드, AWS, 주요 통신사(KT/SKB/LGU+)의 기업용 고정 IP는 공격자뿐만 아니라 정상적인 기업(제휴사)도 사용하는 공용 인프라다. "공격자가 자주 쓰는 대역에 속해 있다"는 사실만으로 정상 IP를 악성으로 분류하는 논리적 비약을 엄격히 금지한다.
- **모바일/CGNAT 주의:** 동일 통신사 IP 또는 동일 /24를 여러 일반 사용자가 시간차로 공유할 수 있다. 동일 IP·인접 대역·동일 통신사라는 사실만으로 특정 개인/조직에 귀속하지 말고, 텔레그램 핸들·게시글 본문·반복 문구·피해 사이트 URL 같은 직접 행위 증거와 결합해서 판단한다.
